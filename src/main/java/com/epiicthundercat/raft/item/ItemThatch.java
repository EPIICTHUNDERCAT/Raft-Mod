	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if (raytraceresult == null) {
			...
			if (flag) {
				...
				ThatchEntity ThatchEntity = new ThatchEntity(worldIn, raytraceresult.hitVec.xCoord,
				...
				if (!worldIn.getCollisionBoxes(ThatchEntity, ThatchEntity.getEntityBoundingBox().expandXyz(-0.1D))
						.isEmpty()) {
					return new ActionResult(EnumActionResult.FAIL, itemStackIn);
				} else {
					if (ConfigHandler.raft.Is_Thatch_Placeable == true) {
						if (!worldIn.isRemote) {
