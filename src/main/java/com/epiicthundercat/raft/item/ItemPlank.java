	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if (raytraceresult == null) {
			...
			if (flag) {
				...
				PlankEntity PlankEntity = new PlankEntity(worldIn, raytraceresult.hitVec.xCoord,
						flag1 ? raytraceresult.hitVec.yCoord - 0.12D : raytraceresult.hitVec.yCoord,
						raytraceresult.hitVec.zCoord);

				PlankEntity.rotationYaw = playerIn.rotationYaw;

				if (!worldIn.getCollisionBoxes(PlankEntity, PlankEntity.getEntityBoundingBox().expandXyz(-0.1D))
						.isEmpty()) {
