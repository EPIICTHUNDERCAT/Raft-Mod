...
import net.minecraftforge.fml.relauncher.SideOnly;
	...
	{
	    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(FloatBarrel.class, DataSerializers.VARINT);
	   private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(FloatBarrel.class, DataSerializers.VARINT);
	    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(FloatBarrel.class, DataSerializers.FLOAT);
	    private static final DataParameter<Integer> Barrel_TYPE = EntityDataManager.<Integer>createKey(FloatBarrel.class, DataSerializers.VARINT);
	    ...
	    /**
	     * How much the Barrel should glide given the slippery blocks it's currently gliding over.
	     * Halved every tick.
	     */
	    ...
	    public FloatBarrel(World worldIn, double x, double y, double z)
	    {
	        this(worldIn);
	     ...
	     * pushable on contact, like Barrels or minecarts.
	     */
	    @Nullable
	    ...
	    {
	        if (entityIn instanceof FloatBarrel)
	        {
	    ...
	    public void onUpdate()
	    {
	        ...
	        this.status = this.getBarrelStatus();

	        if (this.status != FloatBarrel.Status.UNDER_WATER && this.status != FloatBarrel.Status.UNDER_FLOWING_WATER)
	        {
	     ...
	     */
	    private FloatBarrel.Status getBarrelStatus()
	    {
	        FloatBarrel.Status FloatBarrel$status = this.getUnderwaterStatus();

	        if (FloatBarrel$status != null)
	        {
	            this.waterLevel = this.getEntityBoundingBox().maxY;
	            return FloatBarrel$status;
	        }
	        else if (this.checkInWater())
	        {
	            return FloatBarrel.Status.IN_WATER;
	        }
	        else
	        {
	            ...
	            {
	                this.BarrelGlide = f;
	                return FloatBarrel.Status.ON_LAND;
	            }
	            else
	            {
	                return FloatBarrel.Status.IN_AIR;
	            }
	        }
	    }
	    ...
	    /**
	     * Decides how much the Barrel should be gliding on the land (based on any slippery blocks)
	     */
	     ...
	     */
	    @Nullable
	    private FloatBarrel.Status getUnderwaterStatus()
	    {
	        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
	                ...
	                for (int l1 = k; l1 < l; ++l1)
	                {
	                    for (int i2 = i1; i2 < j1; ++i2)
	                    {
	                        ...
	                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
	                        {
	                            ...
	                            {
	                                FloatBarrel.Status FloatBarrel$status = FloatBarrel.Status.UNDER_FLOWING_WATER;
	                                return FloatBarrel$status;
	                            }
	        ...
	        return flag ? FloatBarrel.Status.UNDER_WATER : null;
	    }
	    ...
	    private void updateMotion()
	    {
	        ...
	        if (this.previousStatus == FloatBarrel.Status.IN_AIR && this.status != FloatBarrel.Status.IN_AIR && this.status != FloatBarrel.Status.ON_LAND)
	        {
	            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
	            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
	            this.motionY = 0.0D;
	            this.lastYd = 0.0D;
	            this.status = FloatBarrel.Status.IN_WATER;
	        }
	        else
	        {
	            if (this.status == FloatBarrel.Status.IN_WATER)
	            {
	                d2 = (this.waterLevel - this.getEntityBoundingBox().minY) / (double)this.height;
	                this.momentum = 0.9F;
	                BarrelGlide /= 2f;
	            }
	            else if (this.status == FloatBarrel.Status.UNDER_FLOWING_WATER)
	            {
	                d1 = -7.0E-4D;
	                this.momentum = 0.9F;
	            }
	            else if (this.status == FloatBarrel.Status.UNDER_WATER)
	            {
	                d2 = 0.009999999776482582D;
	                this.momentum = 0.45F;
	            }
	            else if (this.status == FloatBarrel.Status.IN_AIR)
	            {
	                this.momentum = 0.9F;
	            }
	            else if (this.status == FloatBarrel.Status.ON_LAND)
	            {
	 ...
	 /* 
	    public boolean pickup(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing par6, float par7, float par8, float par9)
	    {
	    	 if(!world.isRemote){
	    ...
	    public boolean processInitialInteract(World world, BlockPos pos, EntityPlayer player, @Nullable ItemStack stack, EnumHand hand)
	    {
	    ...
	    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
	    {
	        ...
	        if (!this.isRiding())
	        {
	            if (onGroundIn)
	            {
	                ...
	                {
	                    if (this.status != FloatBarrel.Status.ON_LAND)
	                    {
	                    ...
	                    if (!this.worldObj.isRemote && !this.isDead)
	                    {
	                        ...
	                        if (this.worldObj.getGameRules().getBoolean("doEntityDrops"))
	                        {
	                            for (int i = 0; i < 3; ++i)
	                            {
	                                this.entityDropItem(new ItemStack(Item.getItemFromBlock(Blocks.PLANKS), 1, this.getBarrelType().getMetadata()), 0.0F);
	                            }
	    ...
	    {
	        this.dataManager.set(Barrel_TYPE, Integer.valueOf(BarrelType.ordinal()));
	    }

	    public FloatBarrel.Type getBarrelType()
	    {
	        return FloatBarrel.Type.byId(((Integer)this.dataManager.get(Barrel_TYPE)).intValue());
	    }
	    ...
	    public static enum Type
	    {
	        ...
	        /**
	         * Get a Barrel type by it's enum ordinal
	         */
	        public static FloatBarrel.Type byId(int id)
	        {
	        ...
	        public static FloatBarrel.Type getTypeFromString(String nameIn)
	        {
