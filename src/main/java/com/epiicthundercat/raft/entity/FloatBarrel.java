...
public class FloatBarrel extends Entity {
	...
	private void updateMotion() {
		...
		if (this.previousStatus == FloatBarrel.Status.IN_AIR && this.status != FloatBarrel.Status.IN_AIR
				&& this.status != FloatBarrel.Status.ON_LAND) {
	...
	private void dropItems(World world, BlockPos pos) {
		for (int i = 0; i < MathHelper.getRandomIntegerInRange(world.rand, 3, 10); i++) {
						...
						if (this.worldObj.getGameRules().getBoolean("doEntityDrops")) {
							for (int i = 0; i < 3; ++i) {
								this.entityDropItem(new ItemStack(Item.getItemFromBlock(Blocks.PLANKS), 1,
										this.getBarrelType().getMetadata()), 0.0F);
							}
	...
	private void despawnEntity() {
		if (!getCanDespawn()) {
			...
			Entity entity = this.worldObj.getClosestPlayerToEntity(this, -0.3D);
