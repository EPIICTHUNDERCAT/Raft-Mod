...
public class EntitySharkFemale extends EntityMob{
	...
	private static final DataParameter<Byte> STATUS = EntityDataManager.<Byte>createKey(EntitySharkFemale.class, DataSerializers.BYTE);
    private static final DataParameter<Integer> TARGET_ENTITY = EntityDataManager.<Integer>createKey(EntitySharkFemale.class, DataSerializers.VARINT);
    ...
    protected void initEntityAI()
    {
        ...
        this.targetTasks.addTask(1, new EntityAINearestAttackableTarget(this, EntityLivingBase.class, 10, true, false, new EntitySharkFemale.SharkTargetSelector(this)));
    }
    ...
    {
        EntityLiving.registerFixesMob(fixer, "Female Shark");
    }
    ...
    /**
     * Sets this shark to be an elder or not.
     */
    ...
    {
        return this.isElder() ? (this.isInWater() ? SoundEvents.ENTITY_ELDER_shark_AMBIENT : SoundEvents.ENTITY_ELDERshark_AMBIENTLAND) : (this.isInWater() ? SoundEvents.ENTITY_shark_AMBIENT : SoundEvents.ENTITY_shark_AMBIENT_LAND);
    }

    protected SoundEvent getHurtSound()
    {
        return this.isElder() ? (this.isInWater() ? SoundEvents.ENTITY_ELDER_shark_HURT : SoundEvents.ENTITY_ELDER_shark_HURT_LAND) : (this.isInWater() ? SoundEvents.ENTITY_shark_HURT : SoundEvents.ENTITY_shark_HURT_LAND);
    }

    protected SoundEvent getDeathSound()
    {
        return this.isElder() ? (this.isInWater() ? SoundEvents.ENTITY_ELDER_shark_DEATH : SoundEvents.ENTITY_ELDER_shark_DEATH_LAND) : (this.isInWater() ? SoundEvents.ENTITY_shark_DEATH : SoundEvents.ENTITY_shark_DEATH_LAND);
    }
*/
    /**
...
//        if (this.isshark())
        {
                ...
                for (EntityPlayerMP entityplayermp : this.worldObj.getPlayers(EntityPlayerMP.class, new Predicate<EntityPlayerMP>()
            {
                public boolean apply(@Nullable EntityPlayerMP p_apply_1_)
                    {
                        return EntitySharkFemale.this.getDistanceSqToEntity(p_apply_1_) < 2500.0D && p_apply_1_.interactionManager.survivalOrAdventure();
                    }
                }));
    ...
    static class SharkMoveHelper extends EntityMoveHelper
        {
            ...
            public void onUpdateMoveHelper()
            {
                if (this.action == EntityMoveHelper.Action.MOVE_TO && !this.entitySharkFemale.getNavigator().noPath())
                {
                    ...
                    float f1 = (float)(this.speed * this.entitySharkFemale.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getAttributeValue());
                    this.entitySharkFemale.setAIMoveSpeed(this.entitySharkFemale.getAIMoveSpeed() + (f1 - this.entitySharkFemale.getAIMoveSpeed()) * 0.125F);
                    double d4 = Math.sin((double)(this.entitySharkFemale.ticksExisted + this.entitySharkFemale.getEntityId()) * 0.5D) * 0.05D;
                    ...
                    d4 = Math.sin((double)(this.entitySharkFemale.ticksExisted + this.entitySharkFemale.getEntityId()) * 0.75D) * 0.05D;
                    ...
                    this.entitySharkFemale.getLookHelper().setLookPosition(d10 + (d7 - d10) * 0.125D, d11 + (d8 - d11) * 0.125D, d12 + (d9 - d12) * 0.125D, 10.0F, 40.0F);
