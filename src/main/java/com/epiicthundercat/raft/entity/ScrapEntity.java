...
public class ScrapEntity extends Entity{
	...
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager
			.<Integer>createKey(ScrapEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(ScrapEntity.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Boolean> CAN_DESPAWN = EntityDataManager.createKey(ScrapEntity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> SIZE = EntityDataManager.createKey(ScrapEntity.class,
			DataSerializers.VARINT);
	private static final DataParameter<Boolean> CUSTOM_WIND_ENABLED = EntityDataManager.createKey(ScrapEntity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Float> CUSTOM_WIND_X = EntityDataManager.createKey(ScrapEntity.class,
			DataSerializers.FLOAT);
	...
	private double ScrapPitch;
	private double lerpY;
	private double lerpZ;
	private double ScrapYaw;
	 ...
	 */
	private float ScrapGlide;
	private ScrapEntity.Status status;
	private ScrapEntity.Status previousStatus;
	private double lastYd;

	public ScrapEntity(World worldIn) {
		super(worldIn);
	...
	public ScrapEntity(World worldIn, double x, double y, double z) {
		this(worldIn);
	...
	public void applyEntityCollision(Entity entityIn) {
		if (entityIn instanceof ScrapEntity) {
	...
	private void tickLerp() {
		if (this.lerpSteps > 0) {
			...
			double d3 = MathHelper.wrapDegrees(this.ScrapYaw - (double) this.rotationYaw);
	 ...
	 */
	private ScrapEntity.Status getScrapStatus() {
		ScrapEntity.Status ScrapEntity$status = this.getUnderwaterStatus();

		if (ScrapEntity$status != null) {
			this.waterLevel = this.getEntityBoundingBox().maxY;
			return ScrapEntity$status;
		} else if (this.checkInWater()) {
			return ScrapEntity.Status.IN_WATER;
		} else {
			float f = this.getScrapGlide();

			if (f > 0.0F) {
				this.ScrapGlide = f;
				return ScrapEntity.Status.ON_LAND;
			} else {
				return ScrapEntity.Status.IN_AIR;
			}
		}
	}
	...
	/**
	 * Decides how much the Scrap should be gliding on the land (based on any
	 * slippery blocks)
	 */
	public float getScrapGlide() {
		AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
	 ...
	 */
	@Nullable
	private ScrapEntity.Status getUnderwaterStatus() {
		AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
					...
					for (int i2 = i1; i2 < j1; ++i2) {
						...
						if (iblockstate.getMaterial() == Material.WATER && d0 < (double) getLiquidHeight(iblockstate,
							...
							if (((Integer) iblockstate.getValue(BlockLiquid.LEVEL)).intValue() != 0) {
								ScrapEntity.Status ScrapEntity$status = ScrapEntity.Status.UNDER_FLOWING_WATER;
								return ScrapEntity$status;
							}
		...
		return flag ? ScrapEntity.Status.UNDER_WATER : null;
	}
	...
	/**
	 * Update the Scrap's speed, based on momentum.
	 */
	private void updateMotion() {
		...
		if (this.previousStatus == ScrapEntity.Status.IN_AIR && this.status != ScrapEntity.Status.IN_AIR
				&& this.status != ScrapEntity.Status.ON_LAND) {
			this.waterLevel = this.getEntityBoundingBox().minY + (double) this.height;
			this.setPosition(this.posX, (double) (this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
			this.motionY = 0.0D;
			this.lastYd = 0.0D;
			this.status = ScrapEntity.Status.IN_WATER;
		} else {
			if (this.status == ScrapEntity.Status.IN_WATER) {
				d2 = (this.waterLevel - this.getEntityBoundingBox().minY) / (double) this.height;
				this.momentum = 0.9F;
				ScrapGlide /= 2f;
			} else if (this.status == ScrapEntity.Status.UNDER_FLOWING_WATER) {
				d1 = -7.0E-4D;
				this.momentum = 0.9F;
			} else if (this.status == ScrapEntity.Status.UNDER_WATER) {
				d2 = 0.009999999776482582D;
				this.momentum = 0.45F;
			} else if (this.status == ScrapEntity.Status.IN_AIR) {
				this.momentum = 0.9F;
			} else if (this.status == ScrapEntity.Status.ON_LAND) {
				this.momentum = this.ScrapGlide;
		...
		if (!this.isRiding()) {
			if (onGroundIn) {
				if (this.fallDistance > 3.0F) {
					if (this.status != ScrapEntity.Status.ON_LAND) {
